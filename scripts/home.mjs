...
function start(site) {
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${esc(title)}</title><meta name="description" content="${esc(site.description)}"><meta property="og:type" content="website"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(site.description)}"><meta name="theme-color" content="#f4f0e6"><link rel="icon" href="./assets/favicon.ico"><link rel="apple-touch-icon" sizes="180x180" href="./assets/apple-touch-icon.png"><link rel="preconnect" href="https://cdn.prod.website-files.com" crossorigin><link rel="preload" href="./assets/fonts/wenkai.woff2" as="font" type="font/woff2" crossorigin><link rel="preload" href="./assets/fonts/plex-sans-sc.woff2" as="font" type="font/woff2" crossorigin><link rel="stylesheet" href="./assets/home.css"><script src="./assets/home-trail.js" defer></script><script src="./assets/site.js" defer></script></head><body class="home"><a class="d-skip" href="#main">跳到正文</a>`;
}
function header(site) {
  return `<header class="d-header d-wrap"><a class="d-brand" href="./" aria-label="AI游民首页"><img src="./assets/brand-robot.png" width="40" height="40" alt=""><span>AI游民<small>SHIJIE / KEEP ROAMING</small></span></a><nav aria-label="主导航"><a href="#work">作品</a><a href="#writing">文章</a><a href="#about">关于</a></nav>${link(site.github,'GitHub','d-github')}</header>`;
}
function footer(site) {
  ...
  return `<footer class="d-footer d-wrap"><span>© 2026 谢世杰 · AI ROAM</span>${filings}<a href="./sync-status/">同步状态</a><a href="./about/">个人作品与实践 · 个人观点</a></footer><button class="d-replay" id="replay-trail" type="button" aria-label="回到起点">${icon('arrow-up')}<span>回到起点</span></button></body></html>`;
}
...
function updates(site) {
  const items=[...site.projects.map(p=>({date:p.updated,label:`更新项目介绍 · ${p.name}`,url:`./projects/${p.slug}/`})),...site.articles.map(a=>({date:a.updated,label:`修订文章 · ${a.title}`,url:`./writing/${a.slug}/`}))].sort((a,b)=>b.date.localeCompare(a.date));
...
export function renderHome(site) {
  const l=link,[lord,skills]=site.projects,a=site.articles[0];
  const articleTitle = esc(a.title).replace('，', '，<wbr>');
  return start(site)+header(site)+`
<main id="main">
<section class="h-hero d-wrap"><p class="d-eyebrow">A PERSONAL JOURNAL OF MAKING & LEARNING</p><h1>带着好奇心，<br>把想法<span>做出来。</span></h1><p class="h-hero-description">帮技术人把 AI 变成工作能力、判断力和职业杠杆。<br>这里是我的作品、实践，以及一路探索的记录。</p><div class="d-actions">${l('#journey','沿着线索，开始探索','d-button','arrow-down')}${l('./writing/','先读一篇文章')}</div><div class="h-author"><img src="./assets/brand-robot.png" width="76" height="76" alt="AI游民的朱红机器人"><div><strong>你好，我是谢世杰。</strong><p>开发者，也是一名持续记录的 AI 探索者。</p></div><span class="h-author-mark">AI<br>ROAM.</span></div><div class="h-hero-bottom"><span>一些作品，一些笔记，还有尚未写完的想法。</span><a href="#journey" aria-label="向下探索">${icon('arrow-down')}</a></div></section>
<section class="d-journey d-wrap" id="journey" data-breakpoint="800" aria-label="沿朱红探索线阅读作品与手记"><header class="t-intro h-intro"><p class="d-eyebrow">NOTES ALONG THE WAY</p><h2>一个想法，<br><em>牵出新的可能。</em></h2><p>从做工具，到分享方法，再到一路写下的思考。</p></header><svg id="drawing-trail" class="t-trail" aria-hidden="true"></svg>
  <article class="t-chapter h-chapter" id="work"><div class="t-copy"><p class="d-eyebrow">第一章 / 让想法落地</p><h2>${esc(lord.name)}</h2><p class="d-lead">给多个 Agent，<br>一条清晰的工作路径。</p><p class="d-description">${esc(lord.summary)}${esc(lord.detail)}</p>${l('./projects/agent-lord/','走进这个项目')}</div><div class="h-work-visual"><span class="h-margin-note">一件正在使用的工具</span>${observer()}<div class="h-work-caption">${icon('code')}<span>让每一步执行，都有迹可循。</span></div></div></article>
  <article class="t-chapter t-reverse h-chapter" id="skills"><div class="t-copy"><p class="d-eyebrow">第二章 / 把方法留下</p><h2>${esc(skills.name)}</h2><p class="d-lead">好用的方法，<br>值得被反复使用。</p><p class="d-description">${esc(skills.summary)}${esc(skills.detail)}</p>${l('./projects/dev-skills/','翻开这本工具手册')}</div><div class="h-manual"><div class="h-manual-spine" aria-hidden="true">FIELD KIT · VOL. 01</div><div class="h-manual-content">${icon('book')}<p class="d-eyebrow">开发者手记</p><h3>把经验，<br>写成方法。</h3><div class="h-manual-example"><code>$explain-as-fool</code><p>把复杂的事，讲明白。</p><small>解释一下什么是线程池</small></div><div class="h-manual-example"><code>$review-rules</code><p>让每个判断，有依据。</p><small>复核这条评审意见</small></div><span class="h-manual-bottom">from practice, with care.</span></div></div></article>
  <article class="t-chapter h-chapter" id="writing"><div class="t-copy"><p class="d-eyebrow">第三章 / 留下思考</p><h2>写下来的思考</h2><p class="d-lead">把做过的事，<br>再想清楚一点。</p><p class="d-description">记录 AI 开发中的具体问题、实践方法和判断。从真实的问题出发，在写作中理清思路。</p>${l('./writing/','翻开我的文章')}</div><a class="h-article" href="./writing/${esc(a.slug)}/"><div class="h-article-kicker">${icon('book')}<span>FIELD NOTES / 001</span></div><span class="h-quote" aria-hidden="true">“</span><p class="d-eyebrow">${esc(a.topic)}</p><h3>${articleTitle}</h3><p>${esc(a.summary)}</p><div class="d-article-meta"><time datetime="${a.published}">${date(a.published)}</time><span>${esc(a.reading)} · 阅读</span>${icon('arrow-up-right')}</div></a></article>
  <article class="t-chapter t-reverse h-chapter d-about" id="about"><div class="t-copy"><p class="d-eyebrow">第四章 / 继续探索</p><h2>一条持续的主线</h2><p class="d-lead">工具在变，<br>好奇心一直在。</p><p class="d-description">从研发效能、代码智能到 AI Coding，我一直关心：怎样让开发者工作得更高效。</p>${l('./about/','更多关于我')}</div><div class="h-about-note"><p class="d-eyebrow">A FEW THINGS ALONG THE WAY</p><h3>做一点东西，<br>再向前走一点。</h3><ol><li><span>01</span><div><strong>研发效能</strong><small>让工作更顺畅</small></div></li><li><span>02</span><div><strong>代码智能</strong><small>让工具理解代码</small></div></li><li><span>03</span><div><strong>AI Coding</strong><small>让想法走得更远</small></div></li></ol><span class="h-signature">谢世杰 / AI游民</span></div></article>
</section>
${updates(site)}
